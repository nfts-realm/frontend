...
export const formatBscScanLink = (type, data) => {
  if (type === "Account") {
    ...
    return `https://${BSC_SCAN_PREFIXES[chainId]}bscscan.com/address/${address}`;
  } else {
    const [chainId, hash] = data;
    return `https://${BSC_SCAN_PREFIXES[chainId]}bscscan.com/tx/${hash}`;
  }
};
...
export const calculateAvailableReward = (userActivity) => {
  const playedTimeLevel = Number(userActivity.playedTime / 40) + 1 ? Number(userActivity.playedTime / 40) + 1 : 1;
  const playedPerDayLevel = Number(userActivity.playedPerDay / 4) + 1 ? Number(userActivity.playedPerDay / 4) + 1 : 1;
  const achievementLevel =
    Number(userActivity.achievements / 100) + 1 ? Number(userActivity.achievements / 100) + 1 : 1;
  const challengesLevel = Number(userActivity.challenges / 18) + 1 ? Number(userActivity.challenges / 18) + 1 : 1;
  const userLevel = 1;
  const amountStakedLevel = Number(userActivity.stakedLevel) ? Number(userActivity.stakedLevel) : 1;
...
export const displayFormatTime = (time) => {
  ...
  const hours =
    Number(time / 3600).toFixed(0) > 9 ? Number(time / 3600).toFixed(0) : "0" + Number(time / 3600).toFixed(0);
  const minutes =
    Math.floor(Number(time % 3600) / 60).toFixed(0) > 9
      ? Math.floor(Number(time % 3600) / 60).toFixed(0)
      : "0" + Math.floor(Number(time % 3600) / 60);
  const seconds = Number(time % 60).toFixed(0) > 9 ? Number(time % 60).toFixed(0) : "0" + Number(time % 60).toFixed(0);
