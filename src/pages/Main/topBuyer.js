...
function TopBuyer() {
  ...
  const getBuyers = async () => {
    ...
    for (let i = 0; i < top_buy_list.length; i++) {
      const buy_info = (await firestore.collection("users").doc(top_buy_list[i].userId).get()).data();
      buyers[i] = {
        user_id: i,
        ...top_buy_list[i],
        ...buy_info,
          ...
          {!play && (
            <ul className="sellers-list">
              {collectorLists.map((data, index) => (
                <SellerList key={`seller-collector-${index}`} data={data} id={index} index={index} />
              ))}
            </ul>
          )}
        </div>
