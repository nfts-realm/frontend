...
function TopSeller() {
  ...
  const getSellers = async () => {
    setPlay(true);
    const res = await firestore.collection("history").where("actionType", "==", 1).get();
    let sell_list = [];
    let top_sell_list = [];
    let sellers = [];
    ...
    top_sell_list = filterred.sort((a, b) => parseFloat(b.sum) - parseFloat(a.sum)).slice(0, 10);

    for (let i = 0; i < top_sell_list.length; i++) {
      const sell_info = (await firestore.collection("users").doc(top_sell_list[i].oldUserId).get()).data();
      sellers[i] = {
        user_id: i,
        ...top_sell_list[i],
        ...sell_info,
