...
function EthCard(props) {
  const { price, time, saleType, name, description, creator, paymentType } = props.data;
  ...
  const getUserInfo = async () => {
    const buy_info = (await firestore.collection("users").doc(creator).get()).data();
    setMaker(buy_info);
  };
  ...
  return (
    <div className="eth-card">
      <div className="col-12 mt-3 d-flex justify-content-center items-center">
        <img src={maker?.avatar || "/assets/img/avatars/avatar.jpg"} alt="ftm" className="eth-icon mr-3" />
        <div className="d-flex flex-column">
          <span className="nft-font-22">{maker ? maker.firstName + " " + maker.lastName : "User"}</span>
          <span>{maker ? <Link to={`/creator/${maker.account}`}>{maker.nickName} </Link> : "@user"}</span>
        </div>
      </div>
      ...
      <p className="d-flex justify-content-center nft-font-60 mb-0">{price?.toFixed(2)} FTM</p>
      <p className="d-flex justify-content-end pr-5 nft-color-grey nft-font-20 mb-3">(${(price * rate).toFixed(2)})</p>
      {saleType !== "fix" && (
        <div className="d-flex justify-content-around flex-wrap">
          <div className="d-flex flex-column">
            <div>
              <img src="assets/img/Group192.png" alt="time" className="price-icon mr-1" />
              <span>
                {time === 0
