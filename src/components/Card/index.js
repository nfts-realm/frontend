...
import { setNftSelectedItem } from "store/actions";
...
function Card(props) {
  ...
  const dispatchNftItem = async (payload) => {
    // await dispatch({ type: "NFT_SET_SELECTED", payload });
    ...
    <div className="card">
      {type === "image" ? (
            ...
            {!(time === undefined || time === 0 || time === null) && (
              <div className="card__time card__time--clock">
                ...
                <div className="card__clock">
                  {time < moment().valueOf() ? "Auction is ended" : <Countdown date={time} />}
                </div>
              </div>
            )}
          </Link>
        ...
        <div className="w-full overflow-hidden relative">
          <Link to={`/item/${id}`} className="card__cover audio card__cover--video video">
            <AudioImage
              src={image}
              onClick={(e) => {
                e.preventDefault();
      ...
      <div className={`card__author 'card__author--verified'`}>
        ...
        <Link to={`/creator/${time > 0 && auctionCreator ? auctionCreator : owner}`}>{nickName}</Link>
      </div>
      <div className="card__info">
        ...
        <div className="d-flex ml-auto">
          {imageAttach && (
            <div className="mr-2" data-tip="This NFT Contains Unlockable Content" data-effect="solid">
              {(time === 0 && owner === account) || (time > 0 && auctionCreator === account) ? (
      ...
      <div className="asset__btns">
        <Link to={`/item/${id}`} className="asset__btn asset__btn--full asset__btn--clr height-sm">
